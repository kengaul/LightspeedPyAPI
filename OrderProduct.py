    ...
    try:
        ...
        for product in products:
            ...
            if product.sku in lightspeed_products_dict:
                ...
                if lightspeed_product['has_variants'] is True:
                    if product.isvariant() is True:
                        #print(f'Existing Parent Product {lightspeed_product["variant_parent_id"]}')
                        product.variant_type=lightspeed_product["variant_options"][0]["name"]
                        product.variant_type_value=lightspeed_product["variant_options"][0]["value"]
                        product.set_category(lightspeed_product["product_category"]["name"])
                        ...
                        print(f'This product has variants so you need to define new ones as such {product.sku} {lightspeed_product["sku"]}')
                if lightspeed_product['variant_parent_id'] is None:
                    productid_for_image=lightspeed_product['id']
                    if product.isvariant() is True:
                        #product_as_variant=product.getvariantdata()
                        #print(f"Danger!! Turning Base Product Into Variants!! {product_as_variant} ")
                        #existingparent=lightspeed_api.search_products_by_sku(product.sku)["variant_parent_id"]
                        # Todo - Use old data where appropriate
                        product.name=lightspeed_product["name"]
                        product.is_variant=False
                        if product.supply_price!=lightspeed_product["supply_price"]:
                            product.set_costprice(product.supply_price)
                            print(f"Price on Existing product updated {product.sku} and has inventory: {lightspeed_product['has_inventory']}")
                        if product.price_including_tax>(lightspeed_product["price_including_tax"]*1.05) and lightspeed_product['has_inventory']:
                            print(f"Price Change on stock item of more than 5% {product.sku} {lightspeed_product['price_including_tax']} to {product.price_including_tax}")
                        #product.supplier_code=lightspeed_product["supplier_code"]
                        if len(lightspeed_product["variant_options"]):
                            product.variant_type=lightspeed_product["variant_options"][0]["name"]
                            product.variant_type_value=lightspeed_product["variant_options"][0]["value"]
                        else:
                            product.variant_type=None
                            product.variant_type_value=None
                        product.set_category(lightspeed_product["product_category"]["name"])
                else:
                    # Already a variant so upload impage to parent for use
                    productid_for_image=lightspeed_product['variant_parent_id']
                    #productid_for_image=lightspeed_product['id']
                    product.name=lightspeed_product["name"]
                    if product.supply_price!=lightspeed_product["supply_price"]:
                        product.set_costprice(product.supply_price)
                        print(f"Price on Existing variant updated {product.sku} and has inventory: {lightspeed_product['has_inventory']}")
                    if product.price_including_tax>(lightspeed_product["price_including_tax"]*1.05) and lightspeed_product['has_inventory']:
                        print(f"Price Change on stock item of more than 5% {product.sku}")
                    product.variant_type=lightspeed_product["variant_options"][0]["name"]
                    product.variant_type_value=lightspeed_product["variant_options"][0]["value"]
                    product.set_category(lightspeed_product["product_category"]["name"])
                    #print(lightspeed_product["product_category"])
                #print(f"Existing product - {product.sku}")
            ...
            if exists:
                ...
                image_path = os.path.join(pathname, f"{product.sku}.jpg")
                exists = os.path.isfile(image_path)
                if exists:
                    print("Adding image:", image_path)
                    lightspeed_api.upload_image(productid_for_image, image_path)
